...
/**
 * A postal code standardisation field.
 *
 * The postal code input is validated against the `country_iso` (that can be resolved using `country_iso_resolver`).
 * * If the postal code is valid then the number is formatted in a upper cased and squished format, e.g SW124BC
 * * If the postal code is invalid then its resolved as null.
 *
...
final class PostalCodeType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $normaliser = function (FormEvent $event) use ($options): void {
            ...
            $instance = new PostalCode($data, $iso);
